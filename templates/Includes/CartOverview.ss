					...
					<div id="cart-overview-info">
						<% with Cart %>
							...
							<div class="cart-overview-btns">
								<a href="$Top.CartLink()" class="left btn <% if Top.RedirectCartOverview() %>auto-redirect<% end_if %>" id="cart-summary-btn" alt="Full cart summary">Full Cart Summary</a>
								<a href="$Top.CartLink(Checkout)" class="right btn" id="cart-checkout-btn" alt="Checkout now">Checkout Now</a>
							</div>
						<% end_with %>
					</div>
